...
export async function searchStreams(query, options = {}) {
  ...
  // If cleanChannel looks like a streamer username, fetch that specific channel (supports both live & offline)
  if (cleanChannel && !cleanChannel.startsWith("#") && !excludeSet.has(cleanChannel) && !isLanguageTarget && !isCategoryTarget) {
    fetchTasks.push(getStreamByChannel(cleanChannel, true));
  }

  const results = await Promise.allSettled(fetchTasks);

  // Combine live streams + direct channel, avoiding duplicates
  const streamMap = new Map();
  for (const r of results) {
    if (r.status === "fulfilled" && r.value) {
      if (Array.isArray(r.value)) {
        for (const s of r.value) {
          if (s && !excludeSet.has(s.channelName.toLowerCase())) {
            if (s.isLive || s.channelName.toLowerCase() === cleanChannel) {
      ...
      } else if (typeof r.value === "object" && r.value.channelName && !excludeSet.has(r.value.channelName.toLowerCase())) {
  ...
  // If no search query, return community streams (Yatra first, then Indian GTA RP & Hindi, then global)
  if (!q) {
    try {
      const yatraCommunity = await fetchYatraStreams(10, excludeChannelNames);
      for (const y of yatraCommunity) {
        if (!streamMap.has(y.channelName.toLowerCase())) {
    ...
    return Array.from(streamMap.values())
      .sort((a, b) => {
        ...
        const aIsYatraLive = a.isLive && isYatraStream(a);
        const bIsYatraLive = b.isLive && isYatraStream(b);
        if (aIsYatraLive && !bIsYatraLive) return -1;
        if (!aIsYatraLive && bIsYatraLive) return 1;

        // 2. Other Live streams
        if (a.isLive && !b.isLive) return -1;
        if (!a.isLive && b.isLive) return 1;

        // 3. Indian RP / Hindi streams preference
  ...
  if (q.startsWith("#")) {
    ...
    const hashtagMatches = liveStreams.filter((stream) => {
      ...
      // 2. Specific matching for Yatra Roleplay (#yatraroleplay, #yatra):
      // Must contain 'yatra' in title or tags (e.g. YatraRP, yatra roleplay, YATRA x Nexus ROLEPLAY)
      if (tag === "yatraroleplay" || tag.includes("yatra")) {
        return titleLower.includes("yatra") || tags.some((t) => t.includes("yatra"));
      }
