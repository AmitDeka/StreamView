...
export async function searchStreams(query, options = {}) {
  ...
  // If user searched for a specific channel name directly, query Kick directly
  if (!excludeSet.has(q) && !scored.some(s => s.channelName.toLowerCase() === q)) {
    try {
      const directChannel = await getStreamByChannel(q);
      if (directChannel && directChannel.channelName.toLowerCase() === q) {
