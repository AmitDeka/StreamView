...
export async function searchStreams(query, options = {}) {
  ...
  // If search explicitly involves GTA V, fetch GTA V subcategory
  if (q.includes("gta") || q.includes("grand theft")) {
  ...
  // Strictly return ONLY streams that actually contain this #hashtag!
  // Streams without '#' are strictly excluded (with GTA RP fallback specifically for #yatraroleplay).
  if (q.startsWith("#")) {
    const hashtagMatches = liveStreams.filter((stream) => {
      ...
      return hashtags.includes(q) || titleLower.includes(q);
    });
...
export async function getRelatedStreams(referenceStream, options = {}) {
  ...
  const targetCategory =
    ...
    (filterLower.includes("gta") || filterLower.includes("grand theft") ? "grand-theft-auto-v" : null) ||
  ...
  // Strictly return ONLY streams that actually contain this #hashtag!
  // Streams without '#' are strictly excluded.
  if (filterLower.startsWith("#")) {
    const hashtagMatches = candidateStreams
      .filter((stream) => {
        ...
        return streamHashtags.includes(filterLower) || streamTitleLower.includes(filterLower);
      })
      .sort((a, b) => (b.viewerCount || 0) - (a.viewerCount || 0));
