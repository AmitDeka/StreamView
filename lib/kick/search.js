...
import { isYatraQuery, fetchYatraStreams } from "./yatra.js";
...
export async function searchStreams(query, options = {}) {
  ...
  if (isYatraQuery(q)) {
    return await fetchYatraStreams(limit, excludeChannelNames);
  }
  ...
  // If search involves GTA V, roleplay, or yatra, fetch GTA V subcategory
  if (q.includes("gta") || q.includes("grand theft") || q.includes("yatra") || q.includes("roleplay") || q.includes("rp")) {
    fetchTasks.push(fetchSubcategoryStreams("grand-theft-auto-v", 50));
  }

  // If cleanChannel looks like a streamer username, check if that specific channel is live concurrently
  if (cleanChannel && !cleanChannel.startsWith("#") && !excludeSet.has(cleanChannel) && !isLanguageTarget && !isCategoryTarget) {
    fetchTasks.push(getStreamByChannel(cleanChannel));
  }
...
export async function getRelatedStreams(referenceStream, options = {}) {
  ...
  if (isYatraQuery(activeFilter) || isYatraQuery(activeQuery)) {
    return await fetchYatraStreams(limit, [
      referenceStream.channelName,
      ...excludeChannelNames,
    ]);
  }
  ...
  const targetCategory =
    ...
    (filterLower.includes("gta") || filterLower.includes("grand theft") || filterLower.includes("yatra") || filterLower.includes("rp") ? "grand-theft-auto-v" : null) ||
