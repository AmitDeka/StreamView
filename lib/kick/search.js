...
export async function searchStreams(query, options = {}) {
  ...
  // Resilient fallback: Return top active live streams so users never see a dead empty screen
  return liveStreams
    .sort((a, b) => (b.viewerCount || 0) - (a.viewerCount || 0))
    .slice(0, limit);
}
...
export async function getRelatedStreams(referenceStream, options = {}) {
  ...
  // Determine subcategory or language to fetch for discovery
  const targetCategory =
    ...
    (!filterLower.startsWith("#") && filterLower.length > 2 ? filterLower.replace(/[^a-z0-9]+/g, "-") : null);

  const targetLang = LANGUAGE_CODES[filterLower] || null;
  ...
  const scored = candidateStreams.map((stream) => {
    ...
    if (filterLower) {
      ...
      // Language match
      if (
        (filterLower === "hindi" && (streamLangLower === "hi" || streamLangLower === "hindi")) ||
