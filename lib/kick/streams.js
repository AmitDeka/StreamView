...
/**
 * Fetches live streams for a specific Kick game/subcategory slug.
 */
export async function fetchSubcategoryStreams(slug, limit = 20) {
  if (!slug) return [];
  try {
    ...
    console.warn(`Failed to fetch subcategory streams for ${slug}:`, err.message);
  }
  return [];
}
 ...
 */
export async function fetchLanguageStreams(langCode = "en", limit = 20) {
  ...
  try {
    ...
    console.warn(`Failed to fetch language streams for ${code}:`, err.message);
  }
...
export async function getLiveStreams(limitOrOptions = 30) {
  ...
  try {
    ...
    console.warn("Kick public livestreams request failed:", err.message);
  }
