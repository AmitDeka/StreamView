 ...
 */
export async function getStreamByChannel(channelName, includeOffline = false) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (Array.isArray(rawList) && rawList.length > 0) {
          ...
          const stream = normalizeKickStream(raw);
          if (stream) {
            // Also fetch real user profile image if broadcaster_user_id is available
            if (raw.broadcaster_user_id) {
              try {
                const userRes = await kickFetch(`https://api.kick.com/public/v1/users?id=${raw.broadcaster_user_id}`, {
                  headers: {
                    Authorization: `Bearer ${token}`,
                    Accept: "application/json",
                  },
                ...
                if (userRes.ok) {
                  ...
                  const profilePic = userJson.data?.[0]?.profile_picture;
                  if (profilePic) {
                    stream.avatarUrl = profilePic;
                  }
                }
              } catch (e) {
                // profile fetch failure should not block stream return
              }
            }

            if (includeOffline || stream.isLive) {
              return stream;
            }
          }
          return null;
        } else {
          // Channel definitively does not exist on Kick
          return null;
        }
      }
    } catch (err) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (data && data.slug) {
        // Real-time live status from Kick channel API
        const isLive = Boolean(data.livestream && data.livestream.is_live);

        // If not live and offline not requested, return null
        if (!includeOffline && !isLive) {
          return null;
        }
