...
export function isYatraStream(stream) {
  ...
  // 2. Known Yatra creators: if offline, or if live streaming GTA / roleplay without mentioning another server
...
export async function fetchYatraStreams(
  ...
  const [gtaStreams, hiStreams, ...knownStreams] = await Promise.all([
    ...
    ...channelsToQuery.map((channel) => getStreamByChannel(channel, true)),
  ]);
  ...
  // Prioritize LIVE streams first (sorted by viewers), then OFFLINE community creators
  return yatraList
    .sort((a, b) => {
