...
export function isYatraStream(stream) {
  ...
  const otherServers = ["soulcity", "lifeinsoulcity", "subharambh", "gloryrp", "legacyrp"];
  const isOtherServer = otherServers.some((srv) => title.includes(srv) || tags.some((t) => t.includes(srv)));
  ...
  // To be classified under Yatra Roleplay, the stream MUST actually have 'yatra' in title or tags.
  // Even if a creator is on the known list (like imrocky or stelvin777), if they are currently
  // streaming Soulcity RP (#lifeinsoulcity) or another game, they must not leak into Yatra results.
  if (!hasYatraTitle && !hasYatraTag) {
    return false;
  }

  // Must be GTA / RP related to filter out non-gaming false positives (e.g. religious travel "yatra")
  const isRoleplay =
    title.includes("rp") ||
    ...
    hasYatraTag ||
    dynamicYatraChannels.has(channel);
